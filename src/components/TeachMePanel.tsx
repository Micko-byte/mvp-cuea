...
import { X, BookOpen, Focus, Download, ChevronDown, ChevronUp, AlertCircle, Flame, Loader2 } from "lucide-react";
import { Progress } from "@/components/ui/progress";
import type { TeachMeSession } from "@/types/teachMe";
import { useIsMobile } from "@/hooks/use-mobile";

interface TeachMePanelProps {
  session: TeachMeSession | null;
  loading?: boolean;
  unitName?: string;
...
export const TeachMePanel = ({ session, loading, unitName, onToggleFocusMode, onEndSession, onSendMessage }: TeachMePanelProps) => {
  ...
  const displayName = session?.unitName || unitName || "Unit";
      ...
      <div className="flex items-center justify-between px-3 py-2.5 border-b border-border flex-shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] text-muted-foreground">Setting up...</p>
          </div>
        </div>
        <button onClick={() => { onEndSession(); }} className="p-1.5 rounded-md text-muted-foreground hover:text-foreground hover:bg-muted transition-colors" title="Cancel">
          <X className="w-3.5 h-3.5" />
        </button>
      </div>
      <div className="flex-1 flex flex-col items-center justify-center gap-3 p-6">
        <Loader2 className="w-6 h-6 animate-spin text-primary" />
        <p className="text-sm text-muted-foreground text-center">Scanning your notes and building the topic outline...</p>
        <p className="text-xs text-muted-foreground/60 text-center">This may take a moment</p>
      </div>
    </div>
  );

  const panelBody = !session ? loadingBody : (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-3 py-2.5 border-b border-border flex-shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          <BookOpen className="w-4 h-4 text-primary flex-shrink-0" />
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="text-xs font-semibold text-foreground truncate">
                {!session ? "Setting up..." : activeTopic?.name ?? "Teach Me"}
              </span>
            </div>
            <span className="text-xs font-bold text-muted-foreground flex-shrink-0">
              {!session ? (
                ...
                `Topic ${activeTopicIndex + 1}/${totalTopics}`
              )}
            </span>
          </div>
