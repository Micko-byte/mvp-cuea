        ...
        <div className="max-w-4xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            ...
            <h1 className="text-5xl md:text-6xl font-display font-bold text-foreground leading-tight mb-6">
              Your Academic
              <br />
              <span className="text-gradient-maroon">Success Partner</span>
            </h1>
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto mb-10 leading-relaxed">
              Get instant help with assignments, access course materials, check your schedule, 
              and prepare for exams — all powered by AI trained on your university's curriculum.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <Button variant="outline" size="lg" className="h-12 px-8 text-base border-primary/20 text-primary hover:bg-primary/5">
                Learn More
              </Button>
            </div>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {FEATURES.map((f, i) => (
              <motion.div
                ...
                viewport={{ once: true }}
                className="bg-card rounded-2xl border border-border p-6 shadow-card hover:shadow-lg hover:-translate-y-1 transition-all"
              >
